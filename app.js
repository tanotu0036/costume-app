    ...
    ov.querySelectorAll('[data-setmain]').forEach(el=>{
      el.onclick=async(e)=>{
        ...
        try{
          ...
          S.photos=S.photos.filter(p=>p.衣装id!==S.curCostume.id);
          S.photos.push(...(S.curCostume.写真||[]));
          // 一覧のメイン写真も更新
          const ci=S.costumes.findIndex(c=>c.id===S.curCostume.id);
          if(ci>=0) S.costumes[ci].メイン写真URL=S.curCostume.写真.find(p=>p.種別==='メイン')?.URL||'';
          saveCache({costumes:S.costumes,photos:S.photos,repertoires:S.repertoires,usages:S.usages,settings:S.settings});
          closeModal();
          renderCostumeDetail(document.getElementById('pageBody'));
