...
function renderRepDetail(body){
  ...
  ul.innerHTML=usages.length?usages.map(u=>`
    <div class="tl-item" style="padding:9px 12px">
      ...
      <div style="display:flex;flex-direction:column;gap:4px">
        <button class="hdr-icon" style="color:var(--tx3)" data-eu="${u.id}" data-memo="${esc(u.メモ)}" data-role="${esc(u.役柄)}"><i class="ti ti-pencil"></i></button>
        <button class="hdr-icon" style="color:var(--tx3)" data-du="${u.id}"><i class="ti ti-trash"></i></button>
      </div>
  ...
  ul.querySelectorAll('[data-eu]').forEach(b=>b.onclick=()=>openEditUsage(b.dataset.eu,b.dataset.role,b.dataset.memo));
  ul.querySelectorAll('[data-du]').forEach(b=>b.onclick=()=>confirmDeleteUsage(b.dataset.du));
}
