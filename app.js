  ...
  if(S.listMode==='grid'){
    wrap.innerHTML=`<div class="costume-grid">${list.map(c=>{
      ...
      return `<div class="costume-card" data-id="${c.id}">
        ...
        <div class="costume-meta"><div class="costume-code">${esc(c.衣装ID)}</div><div class="costume-name">${esc(c.衣装名)||'（名称未設定）'}</div><div class="costume-loc"><i class="ti ti-map-pin"></i>${esc(c.保管場所)}</div>${c.対象年齢?`<div style="margin-top:3px"><span style="font-size:9px;background:var(--gd-l);color:var(--gd);border-radius:4px;padding:2px 6px;font-weight:700">${esc(c.対象年齢)}</span></div>`:''}</div>
      </div>`;
    }).join('')}</div>`;
  }else{
    // リスト表示
