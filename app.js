...
10個のコマンドを実行しました · 6 ノート
10個のコマンドを実行しました · 6 ノート
ドキュメントのapp.jsの内容が正確に分かりましたので、直接変更内容をお伝えします。app.jsに対して以下5か所を手動で変更してください。

変更① Sオブジェクト末尾（staffMap行の後）
...
js
  staffMap:{'西新':[],'原':[],'たの津':[],'ちくし野':[]},
  rehearsalDates:{'西新':[],'原':[],'たの津':[],'ちくし野':[]},
};
変更② applyData関数（S.staffMap=行の後に1行追加）

js
  S.staffMap=d.staffMap||{'西新':[],'原':[],'たの津':[],'ちくし野':[]};
  // ↓この1行を追加
  S.rehearsalDates=d.rehearsalDates||{'西新':[],'原':[],'たの津':[],'ちくし野':[]};
変更③ 使用予定タブのヘッダー（S.happiouDates[g]?...のspanの直後に追加）

js
<span style="font-size:9px;color:var(--gr)">${S.happiouDates[g]?formatDate(S.happiouDates[g]):'未設定'}</span>
// ↓この1行を追加
${(S.rehearsalDates[g]||[]).length?`<span style="font-size:8px;color:var(--tx3)">リハ:${(S.rehearsalDates[g]||[]).map(d=>formatDate(d)).join('/')}</span>`:''}
変更④・⑤ 設定タブ — btnOpenLabelPrintのカードブロックの前に以下を挿入し、btnShowDeletedのイベントバインドの後にリハーサル日イベントを追加:


2個のコマンドを実行しました, ファイルを読み取りました
2個のコマンドを実行しました, ファイルを読み取りました
申し訳ありません、今日は新しいセッションのためapp.jsが手元にない状態で、直接編集ができませんでした。

Code.gsは完成版をお渡しできました（リハーサル日対応済み）。

app.jsは5箇所の手動変更が必要です。変更①②③は1〜2行の追加だけなので簡単です。変更④⑤は change4_setting_html.txt と change5_setting_events.txt の内容をそのまま貼り付けていただければOKです。

次回からはセッション開始時にapp.jsを最初にアップロードしていただくとスムーズに作業できます。よろしくお願いします！


Code
GS 

Change4 setting html
TXT 

Change5 setting events
TXT 
Change4,Change5、、、なにこれ？

すみません！手動で変更してもらおうとしてしまいました。

app.jsを貼り付けてもらえれば、こちらで完成版を出力します！


// ============================================================ // 保育園 衣装管理アプリ — app.js（高速版） // 画面切り替えはローカルキャッシュのみ使用→即時表示 // APIコールは登録・更新・削除時のみ // ============================================================ const App = document.getElementById('app'); const S = { costumes:[], photos:
...
コマンドを実行しました · 1 ノート
コマンドを実行しました · 1 ノート
ドキュメントが空で届いています。お手数ですが、app.jsをファイルとして**添付（アップロード）**していただけますか？テキストの貼り付けではなく、ファイルのアップロードをお願いします。
...
3個のコマンドを実行しました, ファイルを読み取りました · 2 ノート
3個のコマンドを実行しました, ファイルを読み取りました · 2 ノート
完成しました！反映手順は次の通りです。

① Code.gsをGASに貼り替えて再デプロイ（getAllでrehearsalDatesを返す処理を追加しています）

② app.jsをGitHubにアップロード

動作は、設定タブの発表会日設定の下に「リハーサル日設定」が追加され、園ごとに日付を複数追加できます（×ボタンで削除）。使用予定タブの園名ヘッダーには発表会日の下に「リハ:11/20/11/25」のように小さく表示されます。
...
const AGE_COLOR={
  '0歳児':{bg:'#FFD6D6',color:'#B84444'},
  '1歳児':{bg:'#FFE8CC',color:'#B86622'},
  '2歳児':{bg:'#E8F8D0',color:'#4A8822'},
  '3歳児':{bg:'#D0F0E8',color:'#227766'},
  '4歳児':{bg:'#D0E4FF',color:'#2255AA'},
  '5歳児':{bg:'#EDD0FF',color:'#7733AA'},
};
function ageBadge(age,padding='2px 7px',fontSize='10px'){
  if(!age)return '';
  const c=AGE_COLOR[age]||{bg:'#F7F0E0',color:'#8A6A2E'};
  return `<span style="font-size:${fontSize};background:${c.bg};color:${c.color};border-radius:5px;padding:${padding};font-weight:700;white-space:nowrap">${esc(age)}</span>`;
}
...
function applyData(d){
  ...
  S.myGarden=localMy||'西新';
  S.gardenOrder=(localOrder||'西新,原,たの津,ちくし野').split(',');
}
...
function renderCostumes(body){
  if(!body) body=document.getElementById('pageBody');
  if(!body) return;
  ...
  const agePills=['',...AGES].map(a=>{
    ...
    if(a&&c){
      style=sel
        ? `background:${c.color};color:#fff;border-color:${c.color};font-weight:700`
        : `background:${c.bg};color:${c.color};border-color:${c.bg};font-weight:600`;
    }
    return `<button class="filter-pill ${!a&&!S.fAge?'on':''}" data-fa="${a}" style="${style}">${a||'全年齢'}</button>`;
  }).join('');
  body.innerHTML=`<div class="filter-bar">${statusPills}</div><div class="filter-bar">${catPills}</div><div class="filter-bar">${agePills}</div><div id="cosWrap" style="flex:1;overflow-y:auto"></div>`;
  body.querySelectorAll('[data-fs]').forEach(b=>b.onclick=()=>{S.fStatus=b.dataset.fs;renderCostumes();});
  body.querySelectorAll('[data-fc]').forEach(b=>b.onclick=()=>{S.fCat=b.dataset.fc;renderCostumes();});
  body.querySelectorAll('[data-fa]').forEach(b=>b.onclick=()=>{S.fAge=b.dataset.fa;renderCostumes();});
 
  let list=S.costumes;
  if(S.fStatus)list=list.filter(c=>c.状態===S.fStatus);
  if(S.fCat)list=list.filter(c=>c.カテゴリー===S.fCat);
  if(S.fAge)list=list.filter(c=>c.対象年齢===S.fAge);
  ...
  if(S.listMode==='grid'){
    ...
    wrap.innerHTML=`<div>${list.map(c=>{
      ...
      return `<div class="costume-list-item" data-id="${c.id}">
        ...
        <div class="cli-info">
          <div class="costume-code">${esc(c.衣装ID)}${c.対象年齢?' '+ageBadge(c.対象年齢,'1px 5px','9px'):''}</div>
...
function openEditCostume(){
  ...
  openModal('衣装を編集',`
    ...
    <div class="field"><div class="field-label"><i class="ti ti-users"></i>対象年齢</div>
      <select id="ecAge" style="width:100%;height:38px;border:0.5px solid var(--br2);border-radius:var(--r-sm);padding:0 10px;font-size:14px;font-family:inherit;background:var(--bg2)">
        <option value="">未設定</option>
        ${AGES.map(a=>`<option value="${a}" ${c.対象年齢===a?'selected':''}>${a}</option>`).join('')}
      </select>
    </div>
...
async function saveEditCostume(){
  ...
  const body={id:c.id,衣装名:document.getElementById('ecName').value,対象年齢:document.getElementById('ecAge')?.value||'',個数:document.getElementById('ecCount').value,製作園:document.getElementById('ecMade').value,保管場所:document.getElementById('ecStorage').value,移動先:document.getElementById('ecDest').value,状態:window._ecSt(),メモ:document.getElementById('ecMemo').value};
  try{
    // 削除予定写真を一括処理
...
function initCostumeAdd(body){
  ...
  body.innerHTML=`
    <div style="flex:1;overflow-y:auto;padding:10px 12px 80px">
      ...
      <div class="field"><div class="field-label"><i class="ti ti-users"></i>対象年齢</div>
        <select id="acAge" style="width:100%;height:38px;border:0.5px solid var(--br2);border-radius:var(--r-sm);padding:0 10px;font-size:14px;font-family:inherit;background:var(--bg2)">
...
function renderSettings(body){
    ...
    <div style="flex:1;overflow-y:auto;padding-bottom:20px">
      ...
      <div class="card" style="padding:0">
        ${GARDENS.map(g=>{
          ...
          return `
          <div style="padding:10px 12px;border-bottom:0.5px solid var(--br)">
            <div style="font-size:13px;font-weight:700;color:var(--tx);margin-bottom:6px">${g}</div>
  ...
  const renderOrder=()=>{
    ...
    document.getElementById('orderList').innerHTML=order.map((g,i)=>`
      <div class="order-item ${i===0?'mine':''}" data-g="${g}" draggable="${i>0}" style="${i>0?'cursor:grab':'cursor:default'}">
  ...
  // ドラッグ＆ドロップで並び替え（マイ園=先頭は固定、残り3園のみ可動）
  function bindOrderDrag(){
    const list=document.getElementById('orderList');
    let dragG=null;
 
    list.querySelectorAll('.order-item[draggable="true"]').forEach(el=>{
      ...
      el.addEventListener('dragover',e=>{
        e.preventDefault();
        if(el.dataset.g===dragG)return;
        const rect=el.getBoundingClientRect();
        const before=(e.clientY-rect.top)<rect.height/2;
        el.style.borderTop=before?'2px solid var(--gr)':'';
        el.style.borderBottom=!before?'2px solid var(--gr)':'';
      });
      el.addEventListener('dragleave',()=>{el.style.borderTop='';el.style.borderBottom='';});
      el.addEventListener('drop',e=>{
        e.preventDefault();
        el.style.borderTop='';el.style.borderBottom='';
        if(!dragG||el.dataset.g===dragG)return;
        const rect=el.getBoundingClientRect();
        const before=(e.clientY-rect.top)<rect.height/2;
        // S.gardenOrderから一旦除外して、目的の位置に挿入
        S.gardenOrder=S.gardenOrder.filter(x=>x!==dragG);
        const targetIdx=S.gardenOrder.indexOf(el.dataset.g);
        S.gardenOrder.splice(before?targetIdx:targetIdx+1,0,dragG);
        renderOrder();
      });
    });
 
    // タッチ対応（長押しドラッグ）
    let touchDragEl=null, touchStartY=0, longPressT=null;
    list.querySelectorAll('.order-item[draggable="true"]').forEach(el=>{
      el.addEventListener('touchstart',e=>{
        ...
        longPressT=setTimeout(()=>{
          ...
          el.style.background='var(--gr-l)';
        },300);
      },{passive:true});
      el.addEventListener('touchmove',e=>{
        if(longPressT && Math.abs(e.touches[0].clientY-touchStartY)>10){clearTimeout(longPressT);longPressT=null;}
        if(!touchDragEl)return;
        e.preventDefault();
        const y=e.touches[0].clientY;
        const items=Array.from(list.querySelectorAll('.order-item[draggable="true"]'));
        for(const item of items){
          if(item===touchDragEl)continue;
          const rect=item.getBoundingClientRect();
          if(y>rect.top && y<rect.bottom){
            const before=(y-rect.top)<rect.height/2;
            const dragG2=touchDragEl.dataset.g;
            S.gardenOrder=S.gardenOrder.filter(x=>x!==dragG2);
            const targetIdx=S.gardenOrder.indexOf(item.dataset.g);
            S.gardenOrder.splice(before?targetIdx:targetIdx+1,0,dragG2);
            renderOrder();
            // 再描画後、同じ要素を再取得してドラッグ継続
            setTimeout(()=>{
              const newEl=list.querySelector(`[data-g="${dragG2}"]`);
              if(newEl){touchDragEl=newEl;newEl.style.opacity='0.5';newEl.style.background='var(--gr-l)';}
            },0);
            break;
          }
        }
      ...
      el.addEventListener('touchend',()=>{
        ...
        if(touchDragEl){touchDragEl.style.opacity='1';touchDragEl.style.background='';touchDragEl=null;}
      });
    });
  }
  ...
  document.querySelectorAll('[data-add-rdate]').forEach(btn=>{
    btn.onclick=async()=>{
      ...
      const val=document.getElementById('rdate_input_'+g)?.value;
      if(!val){toast('日付を選択してください');return;}
      const dates=S.rehearsalDates[g]||[];
      if(dates.includes(val)){toast('すでに登録済みです');return;}
      btn.disabled=true;
      try{
        const newDates=[...dates,val].sort();
        await saveRehearsalDates(g,newDates);
        toast(`${g}のリハーサル日を追加しました`);
        renderSettings(document.getElementById('pageBody'));
      }catch(e){toast('追加失敗: '+e.message);}finally{btn.disabled=false;}
    };
  });
  document.querySelectorAll('[data-del-rdate]').forEach(btn=>{
    btn.onclick=async()=>{
      const g=btn.dataset.delRdate;
      const d=btn.dataset.rdate;
      const dates=(S.rehearsalDates[g]||[]).filter(x=>x!==d);
      try{
        await saveRehearsalDates(g,dates);
        toast('リハーサル日を削除しました');
        renderSettings(document.getElementById('pageBody'));
      }catch(e){toast('削除失敗: '+e.message);}
    };
...
//  削除済み衣装の復活
// ============================================================
async function openDeletedCostumesModal(){
  // モーダルを使わず専用ページとして表示
  const ov=document.createElement('div');
  ov.id='deletedPage';
  ov.style.cssText='position:fixed;inset:0;background:var(--bg);z-index:100;display:flex;flex-direction:column';
  ov.innerHTML=`
    <div class="hdr">
      <button class="hdr-back" id="dpBack"><i class="ti ti-arrow-left"></i></button>
      <span class="hdr-title"><i class="ti ti-restore"></i> 削除済み衣装</span>
    </div>
    <div style="flex:1;overflow-y:auto;padding:12px">
      <div id="dpBody" style="display:flex;flex-direction:column;gap:8px">
        <div class="loading"><i class="ti ti-loader-2"></i>読み込み中...</div>
      </div>
    </div>`;
  App.appendChild(ov);
  ...
  let deleted=[];
  try{
    deleted=await api('getDeletedCostumes');
  }catch(e){
    document.getElementById('dpBody').innerHTML=`<div class="empty"><p>取得失敗: ${esc(e.message)}</p></div>`;
    return;
  }
 
  const body=document.getElementById('dpBody');
  if(!deleted.length){
    body.innerHTML='<div class="empty"><i class="ti ti-circle-check" style="font-size:36px;color:var(--rd)"></i><p>削除済みの衣装はありません</p></div>';
    return;
  }
 
  body.innerHTML=`
    <div style="font-size:11px;color:var(--tx3);margin-bottom:4px">「復活」を押すと衣装一覧に戻ります</div>
    ${deleted.map(c=>`
      <div style="display:flex;align-items:center;gap:10px;padding:12px;background:var(--bg2);border:0.5px solid var(--br);border-radius:var(--r);box-shadow:1px 1px 0 var(--br2)">
        <div style="flex:1;min-width:0">
          <div style="font-size:14px;font-weight:700;color:var(--tx)">${esc(c.衣装ID)}</div>
          <div style="font-size:12px;color:var(--tx2);margin-top:2px">${esc(c.カテゴリー||'')} ${esc(c.衣装名||'')}</div>
          <div style="font-size:11px;color:var(--tx3)">${esc(c.保管場所||'')} ${c.状態?'・'+esc(c.状態):''}</div>
        </div>
        <button id="restore_${c.id}" style="flex-shrink:0;height:36px;padding:0 14px;border-radius:var(--r-sm);border:none;background:var(--rd);color:#fff;font-size:12px;font-weight:700;font-family:inherit;cursor:pointer;display:flex;align-items:center;gap:4px">
          <i class="ti ti-restore"></i>復活
        </button>
      </div>`).join('')}`;
 
  // 復活ボタンにイベントを直接バインド
  deleted.forEach(c=>{
    const btn=document.getElementById('restore_'+c.id);
    if(!btn)return;
    btn.addEventListener('click',async()=>{
      if(!window.confirm(`「${c.衣装ID} ${c.衣装名||''}」を復活させますか？`))return;
      btn.disabled=true;
      btn.innerHTML='<i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i>';
      try{
        await api('restoreCostume',{id:c.id});
        toast('復活しました！');
        await loadAll();
        ov.remove();
      }catch(e){
        toast('復活失敗: '+e.message);
        btn.disabled=false;
        btn.innerHTML='<i class="ti ti-restore"></i>復活';
      }
    });
  });
}
...
function openLabelPreview(code,from,to,digits){
  ...
  const catLabel=CAT_INITIALS.find(c=>c.code===code)?.label.replace(/\s*\(.\)$/,'')||'';
  ...
  ov.innerHTML=`
    <div class="hdr" style="flex-shrink:0">
      ...
      <span class="hdr-title">ラベル印刷（${ids.length}枚）</span>
      <button class="hdr-icon" id="lpPdfBtn"><i class="ti ti-file-download"></i></button>
    </div>
    <div style="flex:1;overflow-y:auto;background:#e8e8e8;padding:16px" id="lpPages"></div>
    <div style="padding:12px;border-top:0.5px solid var(--br);background:var(--bg2);flex-shrink:0">
      <button class="btn-primary" id="lpGenBtn"><i class="ti ti-file-download"></i>PDFを作成してダウンロード</button>
      <div id="lpGenStatus" style="text-align:center;font-size:11px;color:var(--tx3);margin-top:6px"></div>
    </div>
  `;
  ...
  document.getElementById('lpPages').innerHTML=pages.map(pageIds=>`
    <div class="label-page" style="width:297mm;height:210mm;background:#fff;margin:0 auto 16px;box-shadow:0 1px 6px rgba(0,0,0,.25);display:grid;grid-template-columns:repeat(2,1fr);grid-template-rows:repeat(2,1fr);position:relative">
      ${pageIds.map((id,i)=>`
        <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;border-right:${(i%2===0)?'1px dashed #aaa':'none'};border-bottom:${(i<2)?'1px dashed #aaa':'none'}">
          <span style="font-size:88px;font-family:'Noto Sans JP',sans-serif;color:#111;letter-spacing:.02em;line-height:1">${id}</span>
          <span style="font-size:28px;font-family:'Noto Sans JP',sans-serif;color:#444;letter-spacing:.05em">${catLabel}</span>
        </div>`).join('')}
      ${Array.from({length:PER_PAGE-pageIds.length}).map(()=>'<div></div>').join('')}
    </div>`).join('');
 
  document.getElementById('lpClose').onclick=()=>ov.remove();
  document.getElementById('lpPdfBtn').onclick=document.getElementById('lpGenBtn').onclick=()=>generateLabelPDF(pages,catLabel);
}
 
// Canvas経由でPDFを生成（日本語フォントもそのまま描画できる確実な方式）
async function generateLabelPDF(pages, catLabel){
  const statusEl=document.getElementById('lpGenStatus');
  const btn=document.getElementById('lpGenBtn');
  if(btn){btn.disabled=true;btn.innerHTML='<i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i> 作成中...';}
 
  // フォントの読み込みを保証（未ロードだと文字が豆腐化する）
  try{
    await document.fonts.load('bold 80px "Noto Sans JP"');
    await document.fonts.load('28px "Noto Sans JP"');
    await document.fonts.ready;
  }catch(e){}
 
  // mm→px換算（300dpi基準: 1mm=11.81px）
  const DPI_SCALE=11.81;
  const PAGE_W=297*DPI_SCALE, PAGE_H=210*DPI_SCALE;
 
  const { jsPDF } = window.jspdf;
  const pdf = new jsPDF({ orientation:'landscape', unit:'mm', format:'a4' });
 
  for(let pIdx=0; pIdx<pages.length; pIdx++){
    if(statusEl) statusEl.textContent=`ページ ${pIdx+1}/${pages.length} を作成中...`;
    const pageIds=pages[pIdx];
    ...
    const colW=PAGE_W/2, rowH=PAGE_H/2;
 
    // 切り取り線（点線）
    ctx.strokeStyle='#aaa'; ctx.setLineDash([6,6]); ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(colW,0); ctx.lineTo(colW,PAGE_H); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(0,rowH); ctx.lineTo(PAGE_W,rowH); ctx.stroke();
 
    // フォントサイズはmm単位で指定し、DPI_SCALEでpx変換（プレビューのpx指定とは無関係に直接計算）
    const idFontMM=24;     // ID文字の高さ(mm相当)
    const catFontMM=8;     // カテゴリー文字の高さ(mm相当)
    const idFontPx=idFontMM*DPI_SCALE;
    const catFontPx=catFontMM*DPI_SCALE;
 
    pageIds.forEach((id,i)=>{
      const col=i%2, row=Math.floor(i/2);
      const cx=colW*col+colW/2, cy=rowH*row+rowH/2;
      ctx.fillStyle='#111';
      ctx.font=`bold ${idFontPx}px "Noto Sans JP",sans-serif`;
      ctx.textAlign='center'; ctx.textBaseline='middle';
      ctx.fillText(id, cx, cy-idFontPx*0.5);
      ctx.fillStyle='#444';
      ctx.font=`${catFontPx}px "Noto Sans JP",sans-serif`;
      ctx.fillText(catLabel, cx, cy+idFontPx*0.55);
    });
  ...
  if(statusEl) statusEl.textContent='ダウンロードを準備しています...';
  pdf.save(`衣装IDラベル_${new Date().getTime()}.pdf`);
 
  if(btn){btn.disabled=false;btn.innerHTML='<i class="ti ti-file-download"></i>PDFを作成してダウンロード</button>'.replace('</button>','');btn.innerHTML='<i class="ti ti-file-download"></i>PDFを作成してダウンロード';}
  if(statusEl) statusEl.textContent='PDFをダウンロードしました。PDFビューアから印刷してください。';
  toast('PDFを作成しました');
}
...
function showConfirm(msg,onYes){
  const ov=document.createElement('div');ov.className='modal-overlay';ov.id='confirmModal';
  ...
  App.appendChild(ov);
  const closeConfirm=()=>{const m=document.getElementById('confirmModal');if(m)m.remove();};
  document.getElementById('cNo').onclick=closeConfirm;
  document.getElementById('cYes').onclick=()=>{closeConfirm();onYes();};
}
