...
async function openDeletedCostumesModal(){
  ...
  if(btn){btn.disabled=true;btn.innerHTML='<i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i> 読み込み中...';}
  let deleted=[];
  try{
    deleted=await api('getDeletedCostumes');
  }catch(e){
    toast('取得失敗: '+e.message);
    if(btn){btn.disabled=false;btn.innerHTML='<i class="ti ti-eye"></i>削除済み衣装を見る';}
    return;
  }
  if(btn){btn.disabled=false;btn.innerHTML='<i class="ti ti-eye"></i>削除済み衣装を見る';}

  const bodyHTML=deleted.length?`
    <div style="font-size:11px;color:var(--tx3);margin-bottom:10px">タップして復活させる衣装を選んでください</div>
    <div style="display:flex;flex-direction:column;gap:6px">
      ${deleted.map(c=>`
        <div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:var(--bg2);border:0.5px solid var(--br);border-radius:var(--r);cursor:pointer" data-restore="${c.id}" data-name="${esc(c.衣装ID)} ${esc(c.衣装名||'（名称なし）')}">
          <div style="flex:1">
            <div style="font-size:13px;font-weight:700;color:var(--tx)">${esc(c.衣装ID)}</div>
            <div style="font-size:11px;color:var(--tx2)">${esc(c.カテゴリー)} ${esc(c.衣装名||'')}</div>
            <div style="font-size:10px;color:var(--tx3)">${esc(c.保管場所||'')} ${esc(c.状態||'')}</div>
          </div>
          <button class="btn-sub" style="flex-shrink:0;height:32px;padding:0 12px;font-size:11px;background:var(--rd-l);color:var(--rd);border-color:var(--rd-b)" data-restore="${c.id}">
            <i class="ti ti-restore" style="font-size:12px"></i> 復活
          </button>
        </div>`).join('')}
    </div>`
    :'<div class="empty"><i class="ti ti-circle-check" style="font-size:36px;color:var(--rd)"></i><p>削除済みの衣装はありません</p></div>';

  openModal('削除済み衣装',bodyHTML,'閉じる',closeModal,ov=>{
    ov.querySelectorAll('[data-restore]').forEach(el=>{
      el.onclick=async()=>{
        const id=el.dataset.restore;
        const name=el.dataset.name||id;
        showConfirm(`「${name}」を復活させますか？`,async()=>{
          try{
            await api('restoreCostume',{id});
            toast('復活しました！');
            // ローカルデータ更新
            await loadAll();
            closeModal();
          }catch(e){toast('復活失敗: '+e.message);}
        });
      };
    });
  });
