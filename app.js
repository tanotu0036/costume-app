...
function openLabelPrintModal(){
  openModal('衣装IDラベル印刷',`
    ...
    <div style="font-size:11px;color:var(--tx3);margin-top:4px"><i class="ti ti-info-circle" style="font-size:12px"></i> A4横向き・8枚/ページ（切り取り線付き）で印刷されます</div>
  `,'プレビューを開く',()=>{
...
function openLabelPreview(code,from,to,digits){
  ...
  document.getElementById('lpPages').innerHTML=pages.map(pageIds=>`
    <div class="label-page" style="width:297mm;height:210mm;background:#fff;margin:0 auto 16px;box-shadow:0 1px 6px rgba(0,0,0,.25);display:grid;grid-template-columns:repeat(4,1fr);grid-template-rows:repeat(2,1fr);position:relative">
      ${pageIds.map((id,i)=>`
        <div style="display:flex;align-items:center;justify-content:center;border:${(i%4!==3)?'1px dashed #aaa':'none'};border-right:${(i%4!==3)?'1px dashed #aaa':'none'};${i<4?'border-bottom:1px dashed #aaa':''}">
          <span style="font-size:64px;font-family:'Noto Sans JP',sans-serif;color:#111;letter-spacing:.02em">${id}</span>
        </div>`).join('')}
