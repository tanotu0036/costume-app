...
function bindPhotoDelBtns(selector, pendingSet, lblId){
  document.querySelectorAll(selector).forEach(btn=>{
    btn.onclick=()=>{
      const pid=btn.dataset.delph||btn.dataset.drp;
      const wrap=btn.closest('[style*="position:relative"]');
      if(pendingSet.has(pid)){
        pendingSet.delete(pid);
        if(wrap){wrap.style.opacity='1';wrap.querySelector('img,i.ti-hanger,i.ti-photo') && (wrap.style.filter='');}
        btn.style.background='var(--rd)';btn.innerHTML='<i class="ti ti-x"></i>';
      }else{
        pendingSet.add(pid);
        if(wrap){wrap.style.opacity='0.3';wrap.style.filter='grayscale(1)';}
        btn.style.background='var(--gd)';btn.innerHTML='<i class="ti ti-rotate-clockwise"></i>';
      }
...
function openEditCostume(){
  ...
  `,'保存する',saveEditCostume,()=>{
    // afterRender: マーク方式で写真削除イベントをバインド
    bindPhotoDelBtns('[data-delph]', pendingDelPhotos, 'ecPhotoLbl');
  });
...
function openEditRep(){
  ...
  `,'保存する',async()=>{
    ...
    // afterRender: 共通バインド関数で統一
    bindPhotoDelBtns('[data-drp]', pendingDelRepPhotos, 'erPhotoLbl');
  });
}
