...
function renderCostumes(body){
  ...
  const agePills=['',...AGES].map(a=>`<button class="filter-pill ${S.fAge===a?'on':''}" data-fa="${a}">${a||'全年齢'}</button>`).join('');
  ...
  if(S.listMode==='grid'){
    wrap.innerHTML=`<div class="costume-grid">${list.map(c=>{
      ...
      return `<div class="costume-card" data-id="${c.id}">
        ...
        <div class="costume-meta"><div class="costume-code">${esc(c.衣装ID)}</div><div class="costume-name">${esc(c.衣装名)||'（名称未設定）'}</div><div style="display:flex;align-items:center;justify-content:space-between;margin-top:2px"><div class="costume-loc" style="margin-top:0"><i class="ti ti-map-pin"></i>${esc(c.保管場所)}</div>${c.対象年齢?`<span style="font-size:10px;background:var(--gd-l);color:var(--gd);border-radius:5px;padding:2px 7px;font-weight:700;white-space:nowrap">${esc(c.対象年齢)}</span>`:''}</div></div>
      </div>`;
    }).join('')}</div>`;
  }else{
    // リスト表示
    wrap.innerHTML=`<div>${list.map(c=>{
      ...
      return `<div class="costume-list-item" data-id="${c.id}">
        ...
        <div class="cli-info">
          <div class="costume-code">${esc(c.衣装ID)}${c.対象年齢?` <span style="font-size:9px;background:var(--gd-l);color:var(--gd);border-radius:4px;padding:1px 5px;font-weight:700">${esc(c.対象年齢)}</span>`:''}</div>
...
function renderCostumeDetail(body){
  ...
  body.innerHTML=`
    <div style="flex:1;overflow-y:auto">
      ...
      <div class="card">
        ${rowHTML('衣装ID',esc(c.衣装ID))}${rowHTML('カテゴリー',esc(c.カテゴリー))}${rowHTML('対象年齢',c.対象年齢?`<span style="background:var(--gd-l);color:var(--gd);border-radius:4px;padding:2px 8px;font-size:12px;font-weight:700">${esc(c.対象年齢)}</span>`:'')}${rowHTML('衣装名',esc(c.衣装名)||'（未設定）')}${rowHTML('個数',c.個数?esc(c.個数)+'着':'')}${rowHTML('製作園',esc(c.製作園))}${rowHTML('メモ',esc(c.メモ))}
      </div>
