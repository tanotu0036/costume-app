...
function initRepAdd(body){
  ...
  body.innerHTML=`
    <div style="flex:1;overflow-y:auto;padding:10px 12px 80px">
      ...
      <div class="field"><div class="field-label"><i class="ti ti-calendar"></i>年度 <span class="req">必須</span></div>
        <select id="arYearSel" style="width:100%;height:44px;font-size:18px;font-weight:700;border:1.5px solid var(--gr-b);border-radius:var(--r);background:var(--gr-l);color:var(--gr);text-align:center;font-family:inherit;cursor:pointer;outline:none">
          ${YEARS.map(y=>`<option value="${y}" ${y===YEARS[drumIdx]?'selected':''}>${y}年度</option>`).join('')}
        </select>
      </div>
  ...
  // セレクトボックスのイベント（setTimeout で DOM生成後に設定）
  setTimeout(()=>{
    const sel=document.getElementById('arYearSel');
    if(sel){
      sel.value=YEARS[drumIdx];
      sel.onchange=e=>{drumIdx=YEARS.indexOf(parseInt(e.target.value));};
    }
  },0);
}
