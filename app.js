...
async function refreshRepPhotos(repId){
  try{
    ...
    S.curRep.写真=fresh.写真||[];
    S.photos=S.photos.filter(p=>p.演目id!==repId);
    S.photos.push(...(S.curRep.写真));
    ...
    <div style="flex:1;overflow-y:auto;padding:10px 12px 80px">
      ...
      <div class="field"><div class="field-label"><i class="ti ti-calendar"></i>年度 <span class="req">必須</span></div>
        <div class="drum-field">
          <div class="drum-wrap" id="arDrum" style="width:100%"><div class="drum-hl"></div><div class="drum-ft"></div><div class="drum-fb"></div><div class="drum-list" id="arDL"></div></div>
        </div>
      </div>
...
function initDrum(){
  ...
  dw.addEventListener('mousedown',e=>{dDrag=true;dSY=e.clientY;dSO=dCO;e.preventDefault();});
  // touchstartとtouchmoveをnon-passiveにしてページスクロールを防止
  dw.addEventListener('touchstart',e=>{
    dDrag=true;dSY=e.touches[0].clientY;dSO=dCO;
    e.stopPropagation(); // 親要素へのバブリングを防止
  },{passive:false});
  dw.addEventListener('touchmove',e=>{
    if(!dDrag)return;
    e.preventDefault(); // ページスクロール防止
    e.stopPropagation();
    applyDrum(Math.max(-(YEARS.length-1)*IH,Math.min(0,dSO+(e.touches[0].clientY-dSY))),false);
  },{passive:false});
  dw.addEventListener('touchend',e=>{
    if(!dDrag)return;
    dDrag=false;
    drumIdx=Math.max(0,Math.min(YEARS.length-1,Math.round(-dCO/IH)));
    applyDrum(-(drumIdx*IH),true);updDrum();
  },{passive:true});
...
function updDrum(){const dl=document.getElementById('arDL');if(!dl)return;dl.querySelectorAll('.drum-item').forEach((el,i)=>el.classList.toggle('sel',i===drumIdx+1));}
