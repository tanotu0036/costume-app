...
async function openCostumeDetail(id){
  ...
  // バックグラウンドで写真・履歴を取得して更新
  try{
    const full=await CostumeAPI.get(id);
    S.curCostume=full;
    renderCostumeDetail();
...
function renderCostumeDetail(){
  ...
  tr.querySelectorAll('.thumb-item[data-idx]').forEach(t=>{
    t.onclick=()=>{detailPhotoIdx=parseInt(t.dataset.idx);renderCostumeDetail();};
  });
...
function openEditCostume(){
  ...
  const photoSection=photos.length?`
    <div class="field">
      ...
      <div style="font-size:11px;color:var(--tx3);margin-bottom:6px">×ボタンで削除できます</div>
      <div style="display:flex;flex-wrap:wrap;gap:6px" id="ecPhotos">
        ${photos.map(p=>`
          <div style="position:relative;width:70px;height:70px;">
            <div style="width:70px;height:70px;border-radius:8px;overflow:hidden;border:0.5px solid var(--br);background:var(--bg3);display:flex;align-items:center;justify-content:center">
            ...
            <div style="position:absolute;top:-5px;right:-5px;width:20px;height:20px;background:var(--rd);color:#fff;border-radius:50%;border:2px solid #fff;font-size:10px;display:flex;align-items:center;justify-content:center;cursor:pointer" data-delphooto="${p.id}">
              <i class="ti ti-x"></i>
            </div>
  ...
  // 写真削除ボタン
  document.querySelectorAll('[data-delphooto]').forEach(btn=>{
    btn.onclick=async()=>{
      const pid=btn.dataset.delphooto;
      showConfirm('この写真を削除しますか？',async()=>{
        try{
          await PhotoAPI.delete(pid);
          toast('写真を削除しました');
          // 写真一覧をローカルでも更新
          S.curCostume.写真=(S.curCostume.写真||[]).filter(p=>p.id!==pid);
          refreshCostumeInList(c.id);
          closeModal();
          renderCostumeDetail();
          // 少し待ってから編集画面を再表示
...
//  差分更新（高速）
// ============================================================
async function refreshCostumeList(){
  try{
    const d=await AllAPI.get();
    S.costumes=d.costumes||[];
    S.photos=d.photos||[];
