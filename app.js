...
async function generateLabelPDF(pages, catLabel){
  ...
  for(let pIdx=0; pIdx<pages.length; pIdx++){
    ...
    pageIds.forEach((id,i)=>{
      ...
      ctx.fillStyle='#111';
      ctx.font=`bold ${Math.round(88*DPI_SCALE/11.81*1.0)}px "Noto Sans JP",sans-serif`;
      ctx.textAlign='center'; ctx.textBaseline='middle';
      ctx.fillText(id, cx, cy-30);
      ctx.fillStyle='#444';
      ctx.font=`${Math.round(28*DPI_SCALE/11.81*1.0)}px "Noto Sans JP",sans-serif`;
      ctx.fillText(catLabel, cx, cy+70);
    });
