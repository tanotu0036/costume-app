...
async function loadAll(){
  try{
    ...
    S.costumes=d.costumes||[]; S.photos=d.photos||[];
    S.repertoires=d.repertoires||[]; S.usages=d.usages||[];
    S.settings=d.settings||{};
    S.myGarden=S.settings.myGarden||'西新';
    S.gardenOrder=(S.settings.gardenOrder||'西新,原,たの津,ちくし野').split(',');
    S.loaded=true;
  }catch(e){console.error(e);toast('データ取得失敗。再読み込みしてください');}
}
...
function openCostumeDetail(id){
  ...
  // ローカルから写真も取得
  S.curCostume.写真=S.photos.filter(p=>p.衣装id===id).sort((a,b)=>(a.順番||0)-(b.順番||0));
...
function openEditCostume(){
  ...
  const photoSection=photos.length?`
    <div class="field">
      ...
      <div style="font-size:11px;color:var(--tx3);margin-bottom:6px">×で写真を削除できます（確認あり）</div>
      <div style="display:flex;flex-wrap:wrap;gap:8px" id="ecPhotos">
        ${photos.map(p=>`
          <div style="position:relative">
            <div style="width:72px;height:72px;border-radius:8px;overflow:hidden;border:0.5px solid var(--br);background:var(--bg3);display:flex;align-items:center;justify-content:center">
            ...
            <div style="font-size:9px;text-align:center;margin-top:2px;color:var(--tx3)">${esc(p.種別)}</div>
          </div>`).join('')}
      </div>
    </div>`:'';
  ...
  `,'保存する',saveEditCostume);

  // 写真削除ボタン
  document.querySelectorAll('[data-delph]').forEach(btn=>{
    btn.onclick=()=>{
      const pid=btn.dataset.delph;
...
function renderReps(body){
    ...
    <div>
      ...
      ${groups[year].sort((a,b)=>order.indexOf(a.園)-order.indexOf(b.園)).map(r=>`
        <div class="rep-item" data-id="${r.id}">
          <div class="rep-icon-wrap"><i class="ti ti-${r.種別==='劇・劇遊び'?'masks-theater':'music'}"></i></div>
          <div class="rep-info">
  ...
  wrap.querySelectorAll('.rep-card').forEach(it=>it.onclick=()=>openRepDetail(it.dataset.id));
}
...
function openRepDetail(id){
  ...
  S.curRep.写真=S.photos.filter(p=>p.演目id===id).sort((a,b)=>(a.順番||0)-(b.順番||0));
...
function openEditRep(){
      ...
      <div style="display:flex;flex-wrap:wrap;gap:6px">
        ${repPhotos.map(p=>`
          <div style="position:relative;width:70px">
            <div style="width:70px;height:70px;border-radius:8px;overflow:hidden;border:0.5px solid var(--br)">
              ${p.URL?`<img src="${esc(p.URL)}" style="width:100%;height:100%;object-fit:cover">`:''}
            </div>
            <div data-drp="${p.id}" style="position:absolute;top:-5px;right:-5px;width:20px;height:20px;background:var(--rd);color:#fff;border-radius:50%;border:2px solid #fff;font-size:10px;display:flex;align-items:center;justify-content:center;cursor:pointer">
              <i class="ti ti-x"></i>
            </div>
            <div style="font-size:9px;text-align:center;margin-top:2px;color:var(--tx3)">${esc(p.種別)}</div>
          </div>`).join('')}
  ...
  `,'保存する',async()=>{
    ...
    // afterRender: 写真削除マークのイベントバインド
    document.querySelectorAll('[data-drp]').forEach(delBtn=>{
      delBtn.onclick=()=>{
        const pid=delBtn.dataset.drp;
        const wrap=delBtn.closest('[style*="position:relative"]');
        if(pendingDelRepPhotos.has(pid)){
          pendingDelRepPhotos.delete(pid);
          if(wrap)wrap.style.opacity='1';
          delBtn.style.background='var(--rd)';
          delBtn.innerHTML='<i class="ti ti-x"></i>';
        }else{
          pendingDelRepPhotos.add(pid);
          if(wrap)wrap.style.opacity='0.35';
          delBtn.style.background='var(--gd)';
          delBtn.innerHTML='<i class="ti ti-rotate-clockwise"></i>';
        }
        const lbl=document.getElementById('erPhotoLbl');
        if(lbl)lbl.textContent=pendingDelRepPhotos.size>0?`（${pendingDelRepPhotos.size}件を削除予定）`:'';
      };
    });
  });
}
  ...
  body.innerHTML=`
    <div style="flex:1;overflow-y:auto;padding:10px 12px 80px">
      ...
      <div class="field"><div class="field-label"><i class="ti ti-calendar"></i>年度 <span class="req">必須</span></div>
        <div class="drum-field"><span class="drum-val" id="arYV"></span>
          <div class="drum-wrap" id="arDrum"><div class="drum-hl"></div><div class="drum-ft"></div><div class="drum-fb"></div><div class="drum-list" id="arDL"></div></div>
        </div>
      </div>
...
async function submitRep(){
  ...
  setBtn(btn,'<i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i> 登録中...',true);
  try{
    const res=await RepertoireAPI.add({演目名:document.getElementById('arName').value,種別:arState.type,クラス:document.getElementById('arClass').value,年度:YEARS[drumIdx],園:arState.gardens.join('・'),備考:document.getElementById('arMemo').value});
    const newR={id:res.id,演目名:document.getElementById('arName').value,種別:arState.type,クラス:document.getElementById('arClass').value,年度:YEARS[drumIdx],園:arState.gardens.join('・'),備考:document.getElementById('arMemo').value,衣装数:0};
    localAddRep(newR);
    setBtn(btn,'<i class="ti ti-circle-check"></i> 登録完了！',false,'var(--gr2)');
    toast('演目を登録しました');
    setTimeout(goBack,700);
...
let dDrag=false,dSY=0,dSO=0,dCO=0;const IH=32;
function initDrum(){
  ...
  applyDrum(-(drumIdx*IH),false);updDrum();
  const dw=document.getElementById('arDrum');
  dw.addEventListener('mousedown',e=>{dDrag=true;dSY=e.clientY;dSO=dCO;});
  dw.addEventListener('touchstart',e=>{dDrag=true;dSY=e.touches[0].clientY;dSO=dCO;},{passive:true});
  window.addEventListener('mousemove',dMove);window.addEventListener('touchmove',dMoveT,{passive:true});
  window.addEventListener('mouseup',dUp);window.addEventListener('touchend',dUp);
  dw.addEventListener('wheel',e=>{e.preventDefault();drumIdx=Math.max(0,Math.min(YEARS.length-1,drumIdx+(e.deltaY>0?1:-1)));applyDrum(-(drumIdx*IH),true);updDrum();},{passive:false});
}
function applyDrum(off,anim){const dl=document.getElementById('arDL');if(!dl)return;dl.style.transition=anim?'transform .18s':'none';dl.style.transform=`translateY(${off}px)`;dCO=off;}
function dMove(e){if(!dDrag)return;applyDrum(Math.max(-(YEARS.length-1)*IH,Math.min(0,dSO+(e.clientY-dSY))),false);}
function dMoveT(e){if(!dDrag)return;applyDrum(Math.max(-(YEARS.length-1)*IH,Math.min(0,dSO+(e.touches[0].clientY-dSY))),false);}
function dUp(){if(!dDrag)return;dDrag=false;drumIdx=Math.max(0,Math.min(YEARS.length-1,Math.round(-dCO/IH)));applyDrum(-(drumIdx*IH),true);updDrum();}
function updDrum(){const dl=document.getElementById('arDL');if(!dl)return;dl.querySelectorAll('.drum-item').forEach((el,i)=>el.classList.toggle('sel',i===drumIdx+1));const v=document.getElementById('arYV');if(v)v.textContent=YEARS[drumIdx]+'年度';}
...
function openLightbox(photos, startIdx=0){
  ...
  // スワイプ対応
  let tx=0,ty=0;
  ov.addEventListener('touchstart',e=>{tx=e.touches[0].clientX;ty=e.touches[0].clientY;},{passive:true});
  ov.addEventListener('touchend',e=>{
    ...
    if(Math.abs(dy)>80&&Math.abs(dy)>Math.abs(dx))ov.remove();
  },{passive:true});
