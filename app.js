...
function bindPhotoDrag(containerSelector, photos, onReorder, root){
  const scope=root||document.getElementById('activeModal')||document;
  const container=scope.querySelector(containerSelector);
  if(!container)return;

  let dragEl=null,placeholder=null,longPressTimer=null;
  ...
  const startDrag=(el,clientX,clientY)=>{
    ...
    placeholder.style.cssText=`width:72px;height:88px;border:2px dashed var(--gr);border-radius:8px;background:var(--gr-l);flex-shrink:0`;
  ...
  const endDrag=async()=>{
    ...
    // 新しい順番を取得してAPIに送信
    const newOrder=items().map(el=>el.dataset.pid);
    if(onReorder) await onReorder(newOrder);
    dragEl=null;
  };

  // 長押し検出（touchstart + 500ms）
  ...
  container.addEventListener('mousedown',e=>{
    const el=e.target.closest('[data-pid]');
    if(!el||e.target.closest('[data-delph],[data-drp],[data-setmain],[data-setrep]'))return;
    longPressTimer=setTimeout(()=>{longPressTimer=null;startDrag(el,e.clientX,e.clientY);},500);
  });
  window.addEventListener('mousemove',e=>{
    if(longPressTimer){clearTimeout(longPressTimer);longPressTimer=null;}
    if(dragEl)moveDrag(e.clientX,e.clientY);
  });
  window.addEventListener('mouseup',()=>{if(dragEl)endDrag();});
}

// 写真の並び順をAPIに保存
async function savePhotoOrder(newOrder, isRep){
  try{
    for(let i=0;i<newOrder.length;i++){
      const pid=newOrder[i];
      const isMain=i===0;
      await api('updatePhoto',{id:pid,種別:isMain?'メイン':'アングル',順番:i+1});
    }
    toast('並び順を保存しました');
  }catch(e){toast('並び替え保存失敗: '+e.message);}
}
...
function openEditCostume(){
  ...
  `,'保存する',saveEditCostume,(ov)=>{
    ...
    // 長押しドラッグ並び替え（先頭がメインになる）
    bindPhotoDrag('#ecPhotos', photos, async(newOrder)=>{
      await savePhotoOrder(newOrder, false);
      const freshC=await CostumeAPI.get(c.id);
      S.curCostume=freshC;
      S.photos=S.photos.filter(p=>p.衣装id!==c.id);
      S.photos.push(...(freshC.写真||[]));
      const ci=S.costumes.findIndex(x=>x.id===c.id);
      if(ci>=0)S.costumes[ci].メイン写真URL=freshC.写真[0]?.URL||'';
      saveCache({costumes:S.costumes,photos:S.photos,repertoires:S.repertoires,usages:S.usages,settings:S.settings});
      closeModal();detailPhotoIdx=0;renderCostumeDetail(document.getElementById('pageBody'));setTimeout(openEditCostume,50);
    }, ov);
    // メインに設定イベント（タップ）
...
function openEditRep(){
  ...
  `,'保存する',async()=>{
    ...
    // 長押しドラッグ並び替え
    bindPhotoDrag('div[style*="flex-wrap:wrap"]', repPhotos, async(newOrder)=>{
      await savePhotoOrder(newOrder, true);
      await refreshRepPhotos(r.id);
      closeModal();setTimeout(openEditRep,50);
    }, ov);
    // メインに設定イベント（タップ）
